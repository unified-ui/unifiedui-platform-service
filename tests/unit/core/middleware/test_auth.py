...
from unifiedui.core.middleware.apis.v1.auth import _validate_service_key, authenticate, check_permissions
...
class TestAuthenticateWithServiceKey:
    ...
    async def test_authenticate_with_service_key_success(self):
        ...
        mock_request = Mock(spec=Request)
        mock_request.headers = {"Authorization": "Bearer test-token-123", "X-Service-Key": "valid-service-key"}
    ...
    async def test_authenticate_with_service_key_invalid(self):
        """Test that invalid service key raises 403."""
        mock_request = Mock(spec=Request)
        mock_request.headers = {"Authorization": "Bearer test-token-123", "X-Service-Key": "wrong-key"}
        mock_request.state = Mock()
    ...
    async def test_authenticate_with_service_key_valid_but_bearer_invalid(self):
        ...
        mock_request = Mock(spec=Request)
        mock_request.headers = {"Authorization": "Bearer invalid-token", "X-Service-Key": "valid-service-key"}
        mock_request.path_params = {}
        mock_request.state = Mock()
        ...
        with patch("unifiedui.core.middleware.apis.v1.auth.settings") as mock_settings:
            ...
            with (
                patch(
                    "unifiedui.core.middleware.apis.v1.auth.ContextIdentityUser",
                    side_effect=ValueError("Invalid token"),
                ),
                patch("unifiedui.core.middleware.apis.v1.auth.get_db_client"),
            ):
                with patch("unifiedui.core.middleware.apis.v1.auth.get_cache_client"):
                    with pytest.raises(HTTPException) as exc_info:
                        await test_handler(mock_request)
...
class TestAuthenticateDecorator:
    ...
    async def test_authenticate_respects_cache_header(self):
        ...
        mock_request = Mock(spec=Request)
        mock_request.headers = {"Authorization": "Bearer test-token", "X-Use-Cache": "false"}
        mock_request.path_params = {}
        ...
        with (
            patch("unifiedui.core.middleware.apis.v1.auth.ContextIdentityUser", return_value=mock_user),
            patch("unifiedui.core.middleware.apis.v1.auth.get_db_client"),
        ):
            with patch("unifiedui.core.middleware.apis.v1.auth.get_cache_client"):
                await test_handler(mock_request)
    ...
    async def test_authenticate_checks_tenant_access(self):
        ...
        mock_identity.get_id.return_value = "user-123"
        mock_user.identity = mock_identity
        mock_user.tenants = [{"tenant": {"id": "tenant-456"}}]
    ...
    async def test_authenticate_allows_tenant_access(self):
        ...
        mock_identity.get_id.return_value = "user-123"
        mock_user.identity = mock_identity
        mock_user.tenants = [{"tenant": {"id": "tenant-123"}, "roles": ["READER"]}]
    ...
    async def test_authenticate_invalid_token_raises_401(self):
        ...
        with (
            patch(
                "unifiedui.core.middleware.apis.v1.auth.ContextIdentityUser", side_effect=ValueError("Invalid token")
            ),
            patch("unifiedui.core.middleware.apis.v1.auth.get_db_client"),
        ):
            with patch("unifiedui.core.middleware.apis.v1.auth.get_cache_client"):
...
class TestCheckPermissionsDecorator:
    ...
    async def test_check_permissions_tenant_access_denied(self):
        ...
        mock_user = Mock()
        mock_user.tenants = [{"tenant": {"id": "other-tenant"}, "roles": ["READER"]}]
    ...
    async def test_check_permissions_tenant_permission_granted(self):
        ...
        mock_user = Mock()
        mock_user.tenants = [{"tenant": {"id": "tenant-123"}, "roles": [TenantRolesEnum.READER.value]}]
