...
class TestToolRoutes:
    ...
    def test_create_tool_success(
        ...
        tool_data = {
            ...
            "description": "A test MCP server tool",
            "type": "MCP_SERVER"
        }
    ...
    def test_create_tool_openapi_definition(
        self, 
        test_client: TestClient, 
        test_user_token: Any
    ) -> None:
    ...
    def test_list_tools_with_pagination(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        # Test with limit
        response = test_client.get(
            f"{ENDPOINT_TOOLS.format(tenant_id=tenant_id)}?limit=3",
            headers=headers
        )
        ...
        # Test with skip
        response = test_client.get(
            f"{ENDPOINT_TOOLS.format(tenant_id=tenant_id)}?skip=2&limit=2",
            headers=headers
        )
    ...
    def test_list_tools_with_name_filter(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        # Filter by name
        response = test_client.get(
            f"{ENDPOINT_TOOLS.format(tenant_id=tenant_id)}?name_filter=Development",
            headers=headers
        )
    ...
    def test_list_tools_with_quick_list_view(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.get(
            f"{ENDPOINT_TOOLS.format(tenant_id=tenant_id)}?view=quick-list",
            headers=headers
        )
...
class TestToolPrincipals:
    ...
    def test_add_principal_success(self, test_client: TestClient) -> None:
        ...
        test_client.put(
            ...
            json={
                "principal_id": other_user_token.get_id(),
    ...
    def test_get_principal_detail(self, test_client: TestClient) -> None:
        ...
        response = test_client.get(
            ENDPOINT_PRINCIPAL_DETAIL.format(
                tenant_id=tenant_id, 
                tool_id=tool_id, 
                principal_id=owner_token.get_id()
            ),
            headers=owner_headers
        )
