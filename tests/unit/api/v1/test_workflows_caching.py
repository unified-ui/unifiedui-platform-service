...
class TestWorkflowCaching:
    ...
    def test_workflows_list_cached_after_permission_grant(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        list1 = test_client.get(ENDPOINT_WORKFLOWS.format(tenant_id=tenant_id), headers=headers2)
        ...
        list2 = test_client.get(ENDPOINT_WORKFLOWS.format(tenant_id=tenant_id), headers=headers2)
        assert list2.status_code == status.HTTP_200_OK
        assert len(list2.json()) == 1
        assert list2.json()[0]["id"] == agent_id

    def test_workflows_list_cached_after_permission_revoke(self, test_client: TestClient, test_user_token: Any) -> None:
    ...
    def test_workflows_list_cached_after_multiple_permission_changes(
        ...
        list3 = test_client.get(ENDPOINT_WORKFLOWS.format(tenant_id=tenant_id), headers=headers2)
        ...
        list4 = test_client.get(ENDPOINT_WORKFLOWS.format(tenant_id=tenant_id), headers=headers2)
        assert len(list4.json()) == 1
        assert list4.json()[0]["id"] == agent2["id"]

    def test_workflow_detail_cached_correctly(self, test_client: TestClient, test_user_token: Any) -> None:
    ...
    def test_cache_isolation_between_users(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        list1 = test_client.get(ENDPOINT_WORKFLOWS.format(tenant_id=tenant_id), headers=headers1)
        assert len(list1.json()) == 1
        ...
        list4 = test_client.get(ENDPOINT_WORKFLOWS.format(tenant_id=tenant_id), headers=headers1)
        assert len(list4.json()) == 1

    def test_workflow_principals_list_cached_correctly(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        agent_data = {"name": "Test Agent", "description": "Test", "type": AGENT_TYPE_N8N, "config": VALID_N8N_CONFIG}
        create_response = test_client.post(
            ENDPOINT_WORKFLOWS.format(tenant_id=tenant_id), json=agent_data, headers=headers1_no_cache
        )
        agent_id = create_response.json()["id"]
    ...
    def test_cache_header_controls_caching_behavior(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_WORKFLOWS.format(tenant_id=tenant_id), json=agent_data, headers=headers_no_cache
        )
        agent_id = create_response.json()["id"]
...
class TestWorkflowTagCacheInvalidation:
    ...
    def test_removing_tags_invalidates_cache(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        # Remove tags
        test_client.delete(f"/api/v1/platform-service/tenants/{tenant_id}/workflows/{agent_id}/tags", headers=headers)
...
class TestWorkflowListCaching:
    ...
    def test_list_cached_with_order_by(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        test_client.post(ENDPOINT_WORKFLOWS.format(tenant_id=tenant_id), json=agent_data_a, headers=headers)
        test_client.post(ENDPOINT_WORKFLOWS.format(tenant_id=tenant_id), json=agent_data_b, headers=headers)
    ...
    def test_list_cache_key_includes_all_params(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        for params in combos:
            response = test_client.get(ENDPOINT_WORKFLOWS.format(tenant_id=tenant_id), params=params, headers=headers)
