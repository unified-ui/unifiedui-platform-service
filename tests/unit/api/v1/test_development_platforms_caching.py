"""Tests for development platforms caching behavior."""
...
class TestDevelopmentPlatformCaching:
    """Test suite for development platform caching behavior with X-Use-Cache enabled."""
    ...
    def test_creator_permissions_cached(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        user_token = test_client.create_test_user("dp-cache-creator", "DP Cache Creator")
        headers = create_auth_headers(user_token)
        tenant_id = create_tenant_for_user(test_client, user_token)
        
        platform_id = create_development_platform(test_client, tenant_id, headers, "Cached Platform")
        ...
        update_response = test_client.patch(
            ENDPOINT_DEVELOPMENT_PLATFORM_DETAIL.format(tenant_id=tenant_id, development_platform_id=platform_id),
            json={"name": "Updated Cached Platform"},
            headers=headers
        )
    ...
    def test_no_access_cached(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        user_a_token = test_client.create_test_user("dp-cache-a", "DP Cache A")
        ...
        user_b_token = test_client.create_test_user("dp-cache-b", "DP Cache B")
    ...
    def test_permission_grant_invalidates_cache(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        admin_token = test_client.create_test_user("dp-cache-admin-1", "DP Cache Admin 1")
        admin_headers = create_auth_headers(admin_token)
        tenant_id = create_tenant_for_user(test_client, admin_token)
        platform_id = create_development_platform(test_client, tenant_id, admin_headers, "Grant Test")
        
        user_token = test_client.create_test_user("dp-cache-user-1", "DP Cache User 1")
        user_headers = create_auth_headers(user_token)
        
        add_user_to_tenant(test_client, tenant_id, admin_headers, "dp-cache-user-1", "READER")
    ...
    def test_permission_revoke_invalidates_cache(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        admin_token = test_client.create_test_user("dp-cache-admin-2", "DP Cache Admin 2")
        admin_headers = create_auth_headers(admin_token)
        tenant_id = create_tenant_for_user(test_client, admin_token)
        platform_id = create_development_platform(test_client, tenant_id, admin_headers, "Revoke Test")
        
        user_token = test_client.create_test_user("dp-cache-user-2", "DP Cache User 2")
        user_headers = create_auth_headers(user_token)
        
        add_user_to_tenant(test_client, tenant_id, admin_headers, "dp-cache-user-2", "READER")
        
        # Grant ADMIN permission
        test_client.put(
            ENDPOINT_DEVELOPMENT_PLATFORM_PRINCIPALS.format(tenant_id=tenant_id, development_platform_id=platform_id),
            json={
                "principal_id": "dp-cache-user-2",
                "principal_type": PRINCIPAL_TYPE_USER,
                "role": ROLE_ADMIN
            },
            headers=admin_headers
    ...
    def test_development_platforms_list_cached_correctly(self, test_client: TestClient, fake_redis_client: Any) -> None:
        """Test that development platforms list respects permissions and caching."""
        admin_token = test_client.create_test_user("dp-list-admin", "DP List Admin")
        admin_headers = create_auth_headers(admin_token)
        tenant_id = create_tenant_for_user(test_client, admin_token)
        
        # Create multiple platforms
        platform1_id = create_development_platform(test_client, tenant_id, admin_headers, "Platform 1")
        platform2_id = create_development_platform(test_client, tenant_id, admin_headers, "Platform 2")
        platform3_id = create_development_platform(test_client, tenant_id, admin_headers, "Platform 3")
        
        user_token = test_client.create_test_user("dp-list-user", "DP List User")
        user_headers = create_auth_headers(user_token)
        
        add_user_to_tenant(test_client, tenant_id, admin_headers, "dp-list-user", "READER")
        
        # User sees no platforms initially
        response1 = test_client.get(
            ENDPOINT_DEVELOPMENT_PLATFORMS.format(tenant_id=tenant_id),
            headers=user_headers
        )
        ...
        # Grant permission to platforms 1 and 2
        test_client.put(
            ENDPOINT_DEVELOPMENT_PLATFORM_PRINCIPALS.format(tenant_id=tenant_id, development_platform_id=platform1_id),
            json={"principal_id": "dp-list-user", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_READ},
            headers=admin_headers
        )
        test_client.put(
            ENDPOINT_DEVELOPMENT_PLATFORM_PRINCIPALS.format(tenant_id=tenant_id, development_platform_id=platform2_id),
            json={"principal_id": "dp-list-user", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_WRITE},
            headers=admin_headers
        )
        
        # User now sees 2 platforms
        response2 = test_client.get(
            ENDPOINT_DEVELOPMENT_PLATFORMS.format(tenant_id=tenant_id),
            headers=user_headers
        )
        ...
        assert platform3_id not in platform_ids
    ...
    def test_cache_isolated_between_users(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        admin_token = test_client.create_test_user("dp-iso-admin", "DP Isolation Admin")
        admin_headers = create_auth_headers(admin_token)
        tenant_id = create_tenant_for_user(test_client, admin_token)
        platform_id = create_development_platform(test_client, tenant_id, admin_headers, "Isolation Test")
        
        # User A gets READ
        user_a_token = test_client.create_test_user("dp-iso-a", "DP Isolation A")
        user_a_headers = create_auth_headers(user_a_token)
        add_user_to_tenant(test_client, tenant_id, admin_headers, "dp-iso-a", "READER")
        test_client.put(
            ENDPOINT_DEVELOPMENT_PLATFORM_PRINCIPALS.format(tenant_id=tenant_id, development_platform_id=platform_id),
            json={"principal_id": "dp-iso-a", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_READ},
            headers=admin_headers
        )
        
        # User B gets WRITE
        user_b_token = test_client.create_test_user("dp-iso-b", "DP Isolation B")
        user_b_headers = create_auth_headers(user_b_token)
        add_user_to_tenant(test_client, tenant_id, admin_headers, "dp-iso-b", "READER")
        test_client.put(
            ENDPOINT_DEVELOPMENT_PLATFORM_PRINCIPALS.format(tenant_id=tenant_id, development_platform_id=platform_id),
            json={"principal_id": "dp-iso-b", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_WRITE},
            headers=admin_headers
        )
    ...
    def test_tenant_admin_bypass_cached_correctly(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        admin_token = test_client.create_test_user("dp-bypass-admin", "DP Bypass Admin")
        admin_headers = create_auth_headers(admin_token)
        tenant_id = create_tenant_for_user(test_client, admin_token)
        platform_id = create_development_platform(test_client, tenant_id, admin_headers, "Bypass Test")
        
        global_admin_token = test_client.create_test_user("dp-bypass-global", "DP Bypass Global")
        global_admin_headers = create_auth_headers(global_admin_token)
        
        add_user_to_tenant(test_client, tenant_id, admin_headers, "dp-bypass-global", "GLOBAL_ADMIN")
...
class TestDevelopmentPlatformTagCacheInvalidation:
    """Test suite for cache invalidation when adding/removing tags from development platforms."""
    
    def test_adding_tags_invalidates_cache(self, test_client: TestClient, fake_redis_client: Any) -> None:
        """Test that adding tags invalidates the development platform cache."""
        admin_token = test_client.create_test_user("dp-tag-cache-1", "DP Tag Cache 1")
        admin_headers = create_auth_headers(admin_token)
        tenant_id = create_tenant_for_user(test_client, admin_token)
        platform_id = create_development_platform(test_client, tenant_id, admin_headers, "Tagged Platform")
    ...
    def test_removing_tags_invalidates_cache(self, test_client: TestClient, fake_redis_client: Any) -> None:
        """Test that removing tags invalidates the development platform cache."""
        admin_token = test_client.create_test_user("dp-tag-cache-2", "DP Tag Cache 2")
        admin_headers = create_auth_headers(admin_token)
        tenant_id = create_tenant_for_user(test_client, admin_token)
        platform_id = create_development_platform(test_client, tenant_id, admin_headers, "Tagged Platform 2")
    ...
    def test_replacing_tags_invalidates_cache(self, test_client: TestClient, fake_redis_client: Any) -> None:
        """Test that replacing tags invalidates the development platform cache."""
        admin_token = test_client.create_test_user("dp-tag-cache-3", "DP Tag Cache 3")
        admin_headers = create_auth_headers(admin_token)
        tenant_id = create_tenant_for_user(test_client, admin_token)
        platform_id = create_development_platform(test_client, tenant_id, admin_headers, "Tagged Platform 3")
...
class TestDevelopmentPlatformListCaching:
    """Test suite for development platform list caching with order_by, order_direction, and is_active."""
    ...
    def test_list_cached_with_order_by(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        user_token = test_client.create_test_user("dp-list-cache-order", "DP List Cache Order")
        headers = create_auth_headers(user_token)
        tenant_id = create_tenant_for_user(test_client, user_token)
        
        # Create development platforms
        create_development_platform(test_client, tenant_id, headers, "Platform A")
        create_development_platform(test_client, tenant_id, headers, "Platform B")
        ...
        assert response1.json()[0]["name"] == "Platform A"
        ...
        assert response3.json()[0]["name"] == "Platform B"
    ...
    def test_list_cached_with_is_active(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        user_token = test_client.create_test_user("dp-list-cache-active", "DP List Cache Active")
        headers = create_auth_headers(user_token, use_cache=False)
        tenant_id = create_tenant_for_user(test_client, user_token)
        
        # Create two platforms (default is_active=False)
        create_development_platform(test_client, tenant_id, headers, "Platform Inactive")
        platform_id_2 = create_development_platform(test_client, tenant_id, headers, "Platform Active")
        
        # Activate second platform
        test_client.patch(
            ENDPOINT_DEVELOPMENT_PLATFORM_DETAIL.format(tenant_id=tenant_id, development_platform_id=platform_id_2),
            json={"is_active": True},
            headers=headers
        )
        ...
        assert response_active.json()[0]["name"] == "Platform Active"
        ...
        assert response_inactive.json()[0]["name"] == "Platform Inactive"
        
        # Test without is_active (all platforms)
    ...
    def test_list_cache_key_includes_all_params(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        user_token = test_client.create_test_user("dp-list-cache-params", "DP List Cache Params")
        headers = create_auth_headers(user_token)
        tenant_id = create_tenant_for_user(test_client, user_token)
        
        create_development_platform(test_client, tenant_id, headers, "Test Platform")
