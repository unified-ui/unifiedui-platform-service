...
ENDPOINT_PRINCIPAL_DETAIL = "/api/v1/platform-service/tenants/{tenant_id}/conversations/{conversation_id}/principals/{principal_id}"
...
class TestConversationRoutes:
    ...
    def test_list_conversations_with_data(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        test_client.post(
            ENDPOINT_CONVERSATIONS.format(tenant_id=tenant_id),
            json=conversation1_data,
            headers=headers
        )
        test_client.post(
            ENDPOINT_CONVERSATIONS.format(tenant_id=tenant_id),
            json=conversation2_data,
            headers=headers
        )
    ...
    def test_list_conversations_with_pagination(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        # Test with limit
        response = test_client.get(
            f"{ENDPOINT_CONVERSATIONS.format(tenant_id=tenant_id)}?limit=3",
            headers=headers
        )
        ...
        # Test with skip
        response = test_client.get(
            f"{ENDPOINT_CONVERSATIONS.format(tenant_id=tenant_id)}?skip=2&limit=2",
            headers=headers
        )
