...
def test_get_dashboard_stats_empty(test_client: TestClient):
    ...
    assert data["applications"]["inactive"] == 0
...
def test_get_dashboard_stats_with_entities(test_client: TestClient):
    ...
    app1_id = create_application_in_db(test_client, tenant_id, user_id, "App1", is_active=True)
    create_application_in_db(test_client, tenant_id, user_id, "App2", is_active=True)
    create_application_in_db(test_client, tenant_id, user_id, "App3", is_active=False)
    ...
    assert data["applications"]["active"] == 2
    assert data["applications"]["inactive"] == 1
...
def test_get_dashboard_stats_permission_filtering(test_client: TestClient):
    ...
    create_application_in_db(test_client, tenant_id, admin_id, "Admin App1")
    create_application_in_db(test_client, tenant_id, admin_id, "Admin App2")
    ...
    with test_client.db_client.get_session() as session:
        ...
        session.add(app)
        session.commit()
        admin_member = ApplicationMember(
            id=str(uuid.uuid4()),
            tenant_id=tenant_id,
        ...
        regular_member = ApplicationMember(
            id=str(uuid.uuid4()),
            tenant_id=tenant_id,
            application_id=shared_app_id,
            principal_id=regular_id,
    ...
    assert admin_data["applications"]["total"] == 3
    ...
    assert regular_data["applications"]["total"] == 1
