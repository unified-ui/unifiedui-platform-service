...
class TestCredentialRoutes:
    ...
    def test_create_credential_success(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.post(
            ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id), json=credential_data, headers=headers
        )
    ...
    def test_create_credential_missing_name(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.post(
            ...
            json={"description": "Test credential", "credential_type": "API_KEY", "secret_value": "secret"},
            headers=headers,
        )
    ...
    def test_create_credential_invalid_name_type(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.post(
            ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id),
            json={"name": 123, "credential_type": "API_KEY", "secret_value": "secret"},
            headers=headers,
        )
    ...
    def test_create_credential_empty_name(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.post(
            ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id),
            json={"name": "", "credential_type": "API_KEY", "secret_value": "secret"},
            headers=headers,
        )
    ...
    def test_create_credential_missing_secret_value(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.post(
            ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id),
            json={"name": "Test Credential", "credential_type": "API_KEY"},
            headers=headers,
        )
    ...
    def test_create_credential_empty_body(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.post(ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id), json={}, headers=headers)
    ...
    def test_create_credential_without_permission(self, test_client: TestClient) -> None:
        ...
        response = test_client.post(
            ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id),
            json={"name": "Unauthorized Credential", "credential_type": "API_KEY", "secret_value": "secret"},
            headers=headers2,
        )
    ...
    def test_get_credential_not_found(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.get(
            ENDPOINT_CREDENTIAL_DETAIL.format(tenant_id=tenant_id, credential_id=NON_EXISTENT_ID), headers=headers
        )
    ...
    def test_list_credentials_empty(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.get(ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id), headers=headers)
    ...
    def test_list_credentials_with_data(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        cred1_data = {"name": "Credential 1", "credential_type": "API_KEY", "secret_value": "secret1"}
        cred2_data = {"name": "Credential 2", "credential_type": "API_KEY", "secret_value": "secret2"}

        test_client.post(ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id), json=cred1_data, headers=headers)
        test_client.post(ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id), json=cred2_data, headers=headers)
    ...
    def test_list_credentials_with_pagination(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        for i in range(5):
            test_client.post(
                ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id),
                json={"name": f"Credential {i}", "credential_type": "API_KEY", "secret_value": f"secret{i}"},
                headers=headers,
            )

        # Test with limit
        response = test_client.get(f"{ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id)}?limit=3", headers=headers)
        ...
        # Test with skip
        response = test_client.get(
            f"{ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id)}?skip=2&limit=2", headers=headers
        )
    ...
    def test_list_credentials_with_name_filter(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        test_client.post(
            ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id),
            json={"name": "Production API Key", "credential_type": "API_KEY", "secret_value": "prod-secret"},
            headers=headers,
        )
        test_client.post(
            ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id),
            json={"name": "Development Token", "credential_type": "API_KEY", "secret_value": "dev-secret"},
            headers=headers,
        )
        test_client.post(
            ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id),
            json={"name": "QA API Key", "credential_type": "API_KEY", "secret_value": "qa-secret"},
            headers=headers,
        )

        # Filter by name
        response = test_client.get(
            f"{ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id)}?name_filter=Development", headers=headers
        )
    ...
    def test_list_credentials_with_quick_list_view(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.get(
            f"{ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id)}?view=quick-list", headers=headers
        )
    ...
    def test_delete_credential_success(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id),
            json={"name": "To Delete", "credential_type": "API_KEY", "secret_value": "secret"},
            headers=headers,
        )
        credential_id = create_response.json()["id"]
...
class TestCredentialPrincipalRoutes:
    ...
    def test_list_credential_principals_creator_only(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_CREDENTIALS.format(tenant_id=tenant_id),
            json={"name": "Test Credential", "credential_type": "API_KEY", "secret_value": "secret"},
            headers=headers,
        )
        credential_id = create_response.json()["id"]
        ...
        # List principals
        response = test_client.get(
            ENDPOINT_CREDENTIAL_PRINCIPALS.format(tenant_id=tenant_id, credential_id=credential_id), headers=headers
        )
    ...
    def test_get_principal_permissions(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.get(
            ENDPOINT_PRINCIPAL_DETAIL.format(
                tenant_id=tenant_id, credential_id=credential_id, principal_id=test_user_token.get_id()
            ),
            headers=headers,
        )
