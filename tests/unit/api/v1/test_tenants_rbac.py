...
class TestTenantRBAC:
    ...
    def test_creator_becomes_global_admin(self, test_client: TestClient) -> None:
        ...
        response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Test Tenant", "description": "Test"}, headers=headers
        )
    ...
    def test_global_admin_can_update_tenant(self, test_client: TestClient) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Original Name", "description": "Original"}, headers=headers
        )
        tenant_id = create_response.json()["id"]

        # Update tenant
        update_response = test_client.patch(
            ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), json={"name": "Updated Name"}, headers=headers
        )
    ...
    def test_global_admin_can_delete_tenant(self, test_client: TestClient) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "To Delete", "description": "Will be deleted"}, headers=headers
        )
        tenant_id = create_response.json()["id"]

        # Delete tenant
        delete_response = test_client.request(
            "DELETE", ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=headers
        )
    ...
    def test_global_admin_can_manage_principals(self, test_client: TestClient) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Principal Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        add_response = test_client.put(
            ...
            json={"principal_id": "other-user", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_READER},
    ...
    def test_non_member_cannot_access_tenant(self, test_client: TestClient) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Private Tenant", "description": "Only for User A"}, headers=headers_a
        )
        tenant_id = create_response.json()["id"]
        ...
        get_response = test_client.get(ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=headers_b)
        assert get_response.status_code == status.HTTP_403_FORBIDDEN

        # User B cannot update tenant
        update_response = test_client.patch(
            ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), json={"name": "Hacked"}, headers=headers_b
        )
        assert update_response.status_code == status.HTTP_403_FORBIDDEN

        # User B cannot delete tenant
        delete_response = test_client.request(
            "DELETE", ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=headers_b
        )
        ...
        principal_response = test_client.put(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant_id),
            json={"principal_id": "some-user", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_READER},
            headers=headers_b,
        )
    ...
    def test_reader_can_view_but_not_modify(self, test_client: TestClient) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Reader Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        test_client.put(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant_id),
            json={"principal_id": "reader-user", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_READER},
            headers=admin_headers,
        )
        ...
        principals_response = test_client.get(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant_id), headers=reader_headers
        )
        assert principals_response.status_code == status.HTTP_200_OK
    ...
    def test_chat_agents_admin_can_view_but_not_modify_tenant(self, test_client: TestClient) -> None:
        """Test that CHAT_AGENTS_ADMIN role can view but cannot modify tenant structure."""
        # Admin creates tenant
        admin_token = test_client.create_test_user("app-admin", "App Admin")
        admin_headers = create_auth_headers(admin_token, use_cache=False)

        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "App Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        # Admin adds user with CHAT_AGENTS_ADMIN role
        test_client.put(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant_id),
            json={"principal_id": "app-user", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_CHAT_AGENTS_ADMIN},
            headers=admin_headers,
        )
    ...
    def test_multiple_global_admins(self, test_client: TestClient) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Multi Admin", "description": "Test"}, headers=admin1_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        add_user_response = test_client.put(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant_id),
            json={"principal_id": "new-user", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_READER},
            headers=admin2_headers,
        )
        ...
        delete_response = test_client.request(
            "DELETE", ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=admin2_headers
        )
        assert delete_response.status_code == status.HTTP_204_NO_CONTENT
    ...
    def test_user_with_multiple_roles_on_same_tenant(self, test_client: TestClient) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Multi Role", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]

        # Create multi-role user
        test_client.create_test_user("multi-user", "Multi User")

        # Admin adds READER role
        test_client.put(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant_id),
            json={"principal_id": "multi-user", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_READER},
            headers=admin_headers,
        )

        # Admin adds CHAT_AGENTS_ADMIN role
        test_client.put(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant_id),
            json={"principal_id": "multi-user", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_CHAT_AGENTS_ADMIN},
            headers=admin_headers,
        )
        ...
        add_response = test_client.put(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant_id),
            json={"principal_id": "multi-user", "principal_type": PRINCIPAL_TYPE_USER, "role": "CREDENTIALS_ADMIN"},
            headers=admin_headers,
        )
        ...
        assert ROLE_CHAT_AGENTS_ADMIN in roles
    ...
    def test_removing_global_admin_role(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Demote Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        update_response = test_client.patch(
            ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), json={"name": "Updated"}, headers=admin2_headers
        )
        assert update_response.status_code == status.HTTP_200_OK
        ...
        check_response = test_client.get(
            ENDPOINT_PRINCIPAL_DETAIL.format(tenant_id=tenant_id, principal_id="admin-to-demote"), headers=admin_headers
        )
        ...
        update_response2 = test_client.patch(
            ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), json={"name": "Should Fail"}, headers=admin2_headers
        )
    ...
    def test_custom_group_grants_permissions_to_members(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Custom Group Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        access_before = test_client.get(
            ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=regular_user_headers
        )
    ...
    def test_custom_group_with_reader_role_limits_members(
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Reader Group Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
    ...
    def test_user_not_in_custom_group_has_no_group_permissions(
        self, test_client: TestClient, fake_redis_client: Any
    ) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Isolation Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        access_response = test_client.get(
            ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=non_member_headers
        )
        assert access_response.status_code == status.HTTP_403_FORBIDDEN

        # Non-member CANNOT update tenant
        update_response = test_client.patch(
            ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), json={"name": "Should Fail"}, headers=non_member_headers
        )
        assert update_response.status_code == status.HTTP_403_FORBIDDEN

    def test_identity_group_grants_permissions_to_members(
        self, test_client: TestClient, fake_redis_client: Any
    ) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Identity Group Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        access_before = test_client.get(
            ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=group_member_headers
        )
        assert access_before.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_identity_group_with_reader_role_limits_members(
        self, test_client: TestClient, fake_redis_client: Any
    ) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Reader Identity Group Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        reader_token = test_client.create_test_user("ig-reader-user", "IG Reader User", idp_groups=[reader_group_id])
        reader_headers = create_auth_headers(reader_token, use_cache=False)

        # Admin grants READER role to the identity group
        grant_response = test_client.put(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant_id),
            json={"principal_id": reader_group_id, "principal_type": PRINCIPAL_TYPE_GROUP, "role": ROLE_READER},
            headers=admin_headers,
        )
        assert grant_response.status_code == status.HTTP_200_OK
    ...
    def test_user_not_in_identity_group_has_no_group_permissions(
        self, test_client: TestClient, fake_redis_client: Any
    ) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "IG Isolation Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        test_client.create_test_user("ig-member", "IG Member", idp_groups=[privileged_group_id])
    ...
    def test_user_with_multiple_identity_groups(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Multi IG Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        multi_group_token = test_client.create_test_user(
            "ig-multi-user", "IG Multi User", idp_groups=[reader_group_id, apps_admin_group_id]
        )
        ...
        get_response = test_client.get(ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=multi_group_headers)
        assert get_response.status_code == status.HTTP_200_OK

        # User still CANNOT update tenant (neither READER nor CHAT_AGENTS_ADMIN grants this)
        update_response = test_client.patch(
            ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id),
            json={"name": "Should Fail"},
    ...
    def test_identity_group_and_direct_user_permissions_combine(
        self, test_client: TestClient, fake_redis_client: Any
    ) -> None:
        ...
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Combo Permissions Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        combo_user_token = test_client.create_test_user("ig-combo-user", "IG Combo User", idp_groups=[reader_group_id])
