"""Tests for chat agents API endpoints."""
...
def get_valid_n8n_config() -> dict:
    """Returns a valid N8N chat agent config for testing."""
    return {
        "api_version": "v1",
...
class TestChatAgentRoutes:
    ...
    def test_create_chat_agent_success(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test successful chat agent creation."""
        ...
        # Create chat agent with valid N8N config
        n8n_config = get_valid_n8n_config()
        app_data = {"name": "Test Chat Agent", "description": "A test chat agent", "type": "N8N", "config": n8n_config}

        response = test_client.post(ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id), json=app_data, headers=headers)
    ...
    def test_create_chat_agent_missing_name(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test chat agent creation with missing name."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        response = test_client.post(
            ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id), json={"description": "Test app"}, headers=headers
        )

        assert response.status_code == 422

    def test_create_chat_agent_invalid_name_type(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test chat agent creation with invalid name type."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        response = test_client.post(
            ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id), json={"name": 123, "description": "Test"}, headers=headers
        )

        assert response.status_code == 422

    def test_create_chat_agent_empty_name(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test chat agent creation with empty name."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        response = test_client.post(
            ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id), json={"name": "", "description": "Test"}, headers=headers
        )

        assert response.status_code == 422

    def test_create_chat_agent_invalid_description_type(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test chat agent creation with invalid description type."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)
    ...
    def test_create_chat_agent_empty_body(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test chat agent creation with empty JSON body."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        response = test_client.post(ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id), json={}, headers=headers)

        assert response.status_code == 422

    def test_create_chat_agent_without_permission(self, test_client: TestClient) -> None:
        """Test that user without CHAT_AGENT_CREATOR permission cannot create chat agents."""
        ...
        # Try to create chat agent as user2 (should fail - no tenant membership)
        response = test_client.post(
            ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id),
    ...
    def test_get_chat_agent_success(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test successful chat agent retrieval."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        # Create a chat agent
        app_data = {"name": "Test Chat Agent", "description": "Test description", "type": "N8N"}
        create_response = test_client.post(
            ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id), json=app_data, headers=headers
        )
        app_id = create_response.json()["id"]

        # Retrieve the chat agent
    ...
    def test_get_chat_agent_not_found(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test chat agent retrieval with non-existent ID."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        response = test_client.get(
            ENDPOINT_CHAT_AGENT_DETAIL.format(tenant_id=tenant_id, chat_agent_id=NON_EXISTENT_ID), headers=headers
        )

        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_list_chat_agents_empty(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test listing chat agents when none exist."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        response = test_client.get(ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id), headers=headers)
    ...
    def test_list_chat_agents_with_data(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test listing chat agents with existing data."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)
        ...
        test_client.post(ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id), json=app1_data, headers=headers)
        test_client.post(ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id), json=app2_data, headers=headers)
    ...
    def test_list_chat_agents_with_pagination(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test listing chat agents with pagination parameters."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)
        ...
        # Test with limit
        response = test_client.get(f"{ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id)}?limit=3", headers=headers)
        ...
        # Test with skip
        response = test_client.get(
            f"{ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id)}?skip=2&limit=2", headers=headers
        )
    ...
    def test_list_chat_agents_with_name_filter(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test listing chat agents with name filter."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        # Create chat agents with different names
        test_client.post(
            ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id),
        ...
        # Filter by name
        response = test_client.get(
            f"{ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id)}?name=Production", headers=headers
        )
    ...
    def test_list_chat_agents_with_quick_list_view(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test listing chat agents with quick-list view returns only id and name."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        # Create chat agents with valid N8N config
        n8n_config = get_valid_n8n_config()
        test_client.post(
            ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id),
        ...
        response = test_client.get(
            f"{ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id)}?view=quick-list", headers=headers
        )
    ...
    def test_list_chat_agents_with_order_by_name_asc(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test listing chat agents ordered by name ascending."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        # Create chat agents with different names (in non-alphabetical order)
        test_client.post(
            ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id),
        ...
        response = test_client.get(
            f"{ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id)}?order_by=name&order_direction=asc", headers=headers
        )
    ...
    def test_list_chat_agents_with_order_by_name_desc(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test listing chat agents ordered by name descending."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)
        ...
        response = test_client.get(
            f"{ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id)}?order_by=name&order_direction=desc", headers=headers
        )
    ...
    def test_list_chat_agents_with_order_by_created_at(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test listing chat agents ordered by created_at."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        # Create first chat agent
        ...
        response = test_client.get(
            f"{ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id)}?order_by=created_at&order_direction=desc",
            headers=headers,
        )
    ...
    def test_list_chat_agents_order_by_without_direction_defaults_to_asc(
        ...
        response = test_client.get(f"{ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id)}?order_by=name", headers=headers)
    ...
    def test_list_chat_agents_invalid_order_by_column_ignored(
        self, test_client: TestClient, test_user_token: Any
    ) -> None:
        ...
        response = test_client.get(
            f"{ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id)}?order_by=invalid_column", headers=headers
        )
    ...
    def test_update_chat_agent_success(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test successful chat agent update."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)
    ...
    def test_update_chat_agent_not_found(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test chat agent update with non-existent ID."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        response = test_client.patch(
            ENDPOINT_CHAT_AGENT_DETAIL.format(tenant_id=tenant_id, chat_agent_id=NON_EXISTENT_ID),
            json={"name": "Updated"},
            headers=headers,
        )
    ...
    def test_update_chat_agent_partial(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test partial chat agent update."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)
    ...
    def test_update_chat_agent_is_active(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test updating chat agent is_active status."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)
    ...
    def test_update_chat_agent_type(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test updating chat agent type."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        # Create chat agent with N8N type
        create_response = test_client.post(
            ENDPOINT_CHAT_AGENTS.format(tenant_id=tenant_id),
            json={"name": "Test App", "description": "Test", "type": "N8N"},
            headers=headers,
    ...
    def test_update_chat_agent_config(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test updating chat agent config."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)

        # Create chat agent with initial valid N8N config
    ...
    def test_delete_chat_agent_success(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test successful chat agent deletion."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)
    ...
    def test_delete_chat_agent_not_found(self, test_client: TestClient, test_user_token: Any) -> None:
        """Test chat agent deletion with non-existent ID."""
        tenant_id = create_tenant_for_user(test_client, test_user_token)
        headers = create_auth_headers(test_user_token, use_cache=False)
...
class TestChatAgentPrincipalRoutes:
    """Test suite for chat agent principal/permission management routes."""

    def test_list_chat_agent_principals_creator_only(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        # List principals
        response = test_client.get(
            ENDPOINT_CHAT_AGENT_PRINCIPALS.format(tenant_id=tenant_id, chat_agent_id=app_id), headers=headers
        )
        ...
        assert data["resource_type"] == "chat_agent"
    ...
    def test_get_principal_permissions(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.get(
            ENDPOINT_PRINCIPAL_DETAIL.format(
                tenant_id=tenant_id, chat_agent_id=app_id, principal_id=test_user_token.get_id()
            ),
            headers=headers,
        )
    ...
    def test_set_principal_permission_invalid_role(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        response = test_client.put(
            ENDPOINT_CHAT_AGENT_PRINCIPALS.format(tenant_id=tenant_id, chat_agent_id=app_id),
            json={"principal_id": "some-user", "principal_type": PRINCIPAL_TYPE_USER, "role": "INVALID_ROLE"},
            headers=headers,
        )
    ...
    def test_set_principal_permission_invalid_principal_type(
        ...
        response = test_client.put(
            ENDPOINT_CHAT_AGENT_PRINCIPALS.format(tenant_id=tenant_id, chat_agent_id=app_id),
            json={"principal_id": "some-user", "principal_type": "INVALID_TYPE", "role": ROLE_READ},
            headers=headers,
        )
    ...
    def test_delete_principal_permission(self, test_client: TestClient, test_user_token: Any) -> None:
        ...
        get_response = test_client.get(
            ENDPOINT_PRINCIPAL_DETAIL.format(tenant_id=tenant_id, chat_agent_id=app_id, principal_id="other-user"),
            headers=headers,
        )
