...
class TestIdentityRoutes:
    ...
    def test_get_current_user_success(
        ...
        response = test_client.get(ENDPOINT_IDENTITY_ME, headers=auth_headers)
    ...
    def test_get_current_user_invalid_token(self, test_client: TestClient) -> None:
        ...
        response = test_client.get(ENDPOINT_IDENTITY_ME, headers=headers)
    ...
    def test_get_users_success(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        response = test_client.get(ENDPOINT_IDENTITY_USERS, headers=auth_headers)
    ...
    def test_get_users_with_search(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        response = test_client.get(f"{ENDPOINT_IDENTITY_USERS}?search=test", headers=auth_headers)
    ...
    def test_get_users_with_pagination(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        response = test_client.get(f"{ENDPOINT_IDENTITY_USERS}?top=10", headers=auth_headers)
    ...
    def test_get_users_top_validation(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        # Too low
        response = test_client.get(f"{ENDPOINT_IDENTITY_USERS}?top=0", headers=auth_headers)
        assert response.status_code == 422

        # Too high
        response = test_client.get(f"{ENDPOINT_IDENTITY_USERS}?top=1000", headers=auth_headers)
        assert response.status_code == 422
    ...
    def test_get_groups_success(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        response = test_client.get(ENDPOINT_IDENTITY_GROUPS, headers=auth_headers)
    ...
    def test_get_groups_with_search(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        response = test_client.get(f"{ENDPOINT_IDENTITY_GROUPS}?search=admin", headers=auth_headers)
    ...
    def test_get_groups_with_pagination(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        response = test_client.get(f"{ENDPOINT_IDENTITY_GROUPS}?top=50", headers=auth_headers)
    ...
    def test_get_groups_top_validation(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        """Test that top parameter is validated."""
        # Too low
        response = test_client.get(f"{ENDPOINT_IDENTITY_GROUPS}?top=0", headers=auth_headers)
        assert response.status_code == 422

        # Too high
        response = test_client.get(f"{ENDPOINT_IDENTITY_GROUPS}?top=1000", headers=auth_headers)
        assert response.status_code == 422
    ...
    def test_get_user_by_id_success(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        response = test_client.get(ENDPOINT_IDENTITY_USER_DETAIL.format(user_id=user_id), headers=auth_headers)
    ...
    def test_get_group_by_id_success(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        response = test_client.get(ENDPOINT_IDENTITY_GROUP_DETAIL.format(group_id=group_id), headers=auth_headers)
    ...
    def test_user_with_groups(self, test_client: TestClient) -> None:
        ...
        user_token = test_client.create_test_user(
            "user-with-groups", "User With Groups", idp_groups=["group-1", "group-2", "group-3"]
        )
    ...
    def test_get_users_with_all_parameters(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        response = test_client.get(f"{ENDPOINT_IDENTITY_USERS}?search=test&top=20&next_link=", headers=auth_headers)
    ...
    def test_get_groups_with_all_parameters(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        response = test_client.get(f"{ENDPOINT_IDENTITY_GROUPS}?search=admin&top=30&next_link=", headers=auth_headers)
    ...
    def test_special_characters_in_search(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        ...
        for search_term in search_terms:
            response = test_client.get(f"{ENDPOINT_IDENTITY_USERS}?search={search_term}", headers=auth_headers)
            assert response.status_code == status.HTTP_200_OK
...
class TestRefreshPrincipal:
    ...
    def test_refresh_user_principal_creates_new(
        self, test_client: TestClient, auth_headers: dict[str, str], sample_tenant_data: dict
    ) -> None:
        """Test refreshing a user principal creates a new record if it doesn't exist."""
        # First create a tenant
        tenant_response = test_client.post(
            "/api/v1/platform-service/tenants", headers=auth_headers, json=sample_tenant_data
        )
        ...
        response = test_client.put(
            ...
            json={"tenant_id": tenant_id, "type": "IDENTITY_USER"},
        )
    ...
    def test_refresh_group_principal_creates_new(
        ...
        response = test_client.put(
            ENDPOINT_IDENTITY_PRINCIPAL_REFRESH.format(principal_id=principal_id),
            headers=auth_headers,
            json={"tenant_id": tenant_id, "type": "IDENTITY_GROUP"},
        )
    ...
    def test_refresh_user_principal_updates_existing(
        ...
        def normalize_ts(ts):
            return ts.rstrip("Z") if ts else ts
    ...
    def test_refresh_principal_invalid_type(
        ...
        response = test_client.put(
            ...
            headers=auth_headers,
            json={"tenant_id": tenant_id, "type": "INVALID_TYPE"},
        )
    ...
    def test_refresh_principal_custom_group_not_allowed(
        ...
        response = test_client.put(
            ...
            headers=auth_headers,
            json={"tenant_id": tenant_id, "type": "CUSTOM_GROUP"},
        )

        # Should fail validation (CUSTOM_GROUP is not in the Literal type)
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT

    def test_refresh_principal_missing_tenant_id(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        """Test that missing tenant_id returns validation error."""
        response = test_client.put(
            ENDPOINT_IDENTITY_PRINCIPAL_REFRESH.format(principal_id="test-user"),
            headers=auth_headers,
            json={"type": "IDENTITY_USER"},
        )
    ...
    def test_refresh_principal_unauthenticated(self, test_client: TestClient) -> None:
        ...
        response = test_client.put(
            ENDPOINT_IDENTITY_PRINCIPAL_REFRESH.format(principal_id="test-user"),
            json={"tenant_id": "some-tenant-id", "type": "IDENTITY_USER"},
        )

        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_refresh_principal_empty_body(self, test_client: TestClient, auth_headers: dict[str, str]) -> None:
        """Test that empty request body returns validation error."""
        response = test_client.put(
            ENDPOINT_IDENTITY_PRINCIPAL_REFRESH.format(principal_id="test-user"), headers=auth_headers, json={}
        )
