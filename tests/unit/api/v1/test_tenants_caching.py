...
class TestTenantCaching:
    ...
    def test_custom_group_permission_grant_invalidates_member_cache(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        from unifiedui.core.database.models import Principal, CustomGroupMember, TenantMember
    ...
    def test_identity_group_permission_grant_invalidates_member_cache(self, test_client: TestClient, fake_redis_client: Any) -> None:
    ...
    def test_multiple_permission_changes_invalidate_cache(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        assert test_client.get(ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=user_headers).status_code == status.HTTP_403_FORBIDDEN
        ...
        # User can now read
        assert test_client.get(ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=user_headers).status_code == status.HTTP_200_OK
        # But cannot update
        assert test_client.patch(ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), json={"name": "Fail"}, headers=user_headers).status_code == status.HTTP_403_FORBIDDEN
        ...
        # User can now update
        assert test_client.patch(ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), json={"name": "Success"}, headers=user_headers).status_code == status.HTTP_200_OK
        ...
        assert test_client.patch(ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), json={"name": "Fail2"}, headers=user_headers).status_code == status.HTTP_403_FORBIDDEN
