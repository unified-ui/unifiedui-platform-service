...
class TestUserFavoritesRBAC:
    ...
    def test_user_cannot_delete_other_users_favorites(self, test_client: TestClient) -> None:
        ...
        # Create a chat agent directly in DB and add to user 1's favorites
        app_id = create_chat_agent_in_db(test_client, tenant_id, user_1_id)
        test_client.put(
            ENDPOINT_USER_FAVORITE_DETAIL.format(
...
class TestUserFavoritesIsolation:
    ...
    def test_favorites_isolated_between_users(self, test_client: TestClient) -> None:
        ...
        # Give user 2 access to the chat agent directly in DB
        ...
        test_client.put(
            ENDPOINT_USER_FAVORITE_DETAIL.format(
                tenant_id=tenant_id, user_id=user_2_id, resource_type="chat-agents", resource_id=app_id
            ),
            headers=user_2_headers,
        )
