...
ENDPOINT_APPLICATION_CONFIG = "/api/v1/platform-service/tenants/{tenant_id}/applications/{application_id}/config"
...
class TestApplicationConfigRBAC:
    ...
    def test_config_requires_service_key_header(self, test_client: TestClient) -> None:
        ...
        tenant_id = create_tenant_for_user(test_client, user_token)
        app_id = create_application(test_client, tenant_id, headers, "Config Test App")
    ...
    def test_config_rejects_invalid_service_key(self, test_client: TestClient) -> None:
        ...
        tenant_id = create_tenant_for_user(test_client, user_token)
        app_id = create_application(test_client, tenant_id, base_headers, "Config Test App 2")
    ...
    def test_config_with_valid_service_key_and_read_permission(self, test_client: TestClient) -> None:
        ...
        Note: The endpoint may return 400 if the application config is incomplete (missing credentials),
        ...
        admin_headers = create_auth_headers(admin_token, use_cache=False)
        tenant_id = create_tenant_for_user(test_client, admin_token)
        app_id = create_application(test_client, tenant_id, admin_headers, "Config Test App 3")
        ...
        # Give reader READ permission on application
        test_client.put(
            ENDPOINT_APPLICATION_PRINCIPALS.format(tenant_id=tenant_id, application_id=app_id),
            json={
                "principal_id": "config-reader",
                "principal_type": PRINCIPAL_TYPE_USER,
                "role": ROLE_READ
            },
            headers=admin_headers
    ...
    def test_config_with_valid_service_key_but_no_permission(self, test_client: TestClient) -> None:
        """Test that /config endpoint fails if user has no permission on application."""
        # Create admin user and tenant
        admin_token = test_client.create_test_user("config-admin-2", "Config Admin 2")
        admin_headers = create_auth_headers(admin_token, use_cache=False)
        tenant_id = create_tenant_for_user(test_client, admin_token)
        app_id = create_application(test_client, tenant_id, admin_headers, "Config Test App 4")
        
        # Create another user in tenant but without application permission
    ...
    def test_config_global_admin_bypasses_permission_check(self, test_client: TestClient) -> None:
        """Test that GLOBAL_ADMIN can access /config without explicit application permission.
        ...
        app_id = create_application(test_client, tenant_id, owner_headers, "Config Test App 5")
    ...
    def test_config_applications_admin_bypasses_permission_check(self, test_client: TestClient) -> None:
        """Test that APPLICATIONS_ADMIN can access /config without explicit application permission.
        ...
        owner_headers = create_auth_headers(owner_token, use_cache=False)
        tenant_id = create_tenant_for_user(test_client, owner_token)
        app_id = create_application(test_client, tenant_id, owner_headers, "Config Test App 6")
        
        # Create APPLICATIONS_ADMIN user
        apps_admin_token = test_client.create_test_user("config-apps-admin", "Config Apps Admin")
        add_user_to_tenant(test_client, tenant_id, owner_headers, "config-apps-admin", "APPLICATIONS_ADMIN")
