...
class TestChatWidgetRBAC:
    ...
    def test_creator_becomes_admin(self, test_client: TestClient) -> None:
        ...
        principals_response = test_client.get(
            ENDPOINT_PRINCIPAL_DETAIL.format(tenant_id=tenant_id, chat_widget_id=widget_id, principal_id="cw-creator"),
            headers=headers,
        )
    ...
    def test_admin_can_delete_chat_widget(self, test_client: TestClient) -> None:
        ...
        delete_response = test_client.request(
            "DELETE", ENDPOINT_CHAT_WIDGET_DETAIL.format(tenant_id=tenant_id, chat_widget_id=widget_id), headers=headers
        )
    ...
    def test_read_user_can_view_but_not_modify(self, test_client: TestClient) -> None:
        ...
        test_client.put(
            ENDPOINT_CHAT_WIDGET_PRINCIPALS.format(tenant_id=tenant_id, chat_widget_id=widget_id),
            json={"principal_id": "cw-reader", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_READ},
            headers=admin_headers,
        )

        # Reader CAN view
        get_response = test_client.get(
            ENDPOINT_CHAT_WIDGET_DETAIL.format(tenant_id=tenant_id, chat_widget_id=widget_id), headers=reader_headers
        )
    ...
    def test_write_user_can_modify_but_not_delete_or_manage_principals(self, test_client: TestClient) -> None:
        ...
        test_client.put(
            ENDPOINT_CHAT_WIDGET_PRINCIPALS.format(tenant_id=tenant_id, chat_widget_id=widget_id),
            json={"principal_id": "cw-writer", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_WRITE},
            headers=admin_headers,
        )

        # Writer CAN view
        get_response = test_client.get(
            ENDPOINT_CHAT_WIDGET_DETAIL.format(tenant_id=tenant_id, chat_widget_id=widget_id), headers=writer_headers
        )
        assert get_response.status_code == status.HTTP_200_OK
    ...
    def test_multiple_admins(self, test_client: TestClient) -> None:
        ...
        test_client.put(
            ENDPOINT_CHAT_WIDGET_PRINCIPALS.format(tenant_id=tenant_id, chat_widget_id=widget_id),
            json={"principal_id": "cw-admin-2", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_ADMIN},
    ...
    def test_tenant_chat_widgets_admin_bypasses_permissions(self, test_client: TestClient) -> None:
        ...
        get_response = test_client.get(
            ENDPOINT_CHAT_WIDGET_DETAIL.format(tenant_id=tenant_id, chat_widget_id=widget_id), headers=cw_admin_headers
        )
        assert get_response.status_code == status.HTTP_200_OK
    ...
    def test_user_role_replacement(self, test_client: TestClient) -> None:
        ...
        get_response = test_client.get(
            ENDPOINT_PRINCIPAL_DETAIL.format(tenant_id=tenant_id, chat_widget_id=widget_id, principal_id="test-user"),
            headers=admin_headers,
        )
    ...
    def test_removing_admin_role(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        test_client.put(
            ENDPOINT_CHAT_WIDGET_PRINCIPALS.format(tenant_id=tenant_id, chat_widget_id=widget_id),
            json={"principal_id": "cw-temp-admin", "principal_type": PRINCIPAL_TYPE_USER, "role": ROLE_ADMIN},
            headers=admin_headers,
        )
