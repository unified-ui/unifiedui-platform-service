...
class TestIdentityCaching:
    ...
    def test_user_groups_cached(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        user_token = test_client.create_test_user(
            "cache-groups-user", "Cache Groups User", idp_groups=["group-a", "group-b", "group-c"]
        )
        headers = create_auth_headers(user_token)

        # Create tenant to trigger group cache
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Groups Cache Test", "description": "Test"}, headers=headers
        )
        ...
        grant_response = test_client.put(
            ...
            json={"principal_id": "group-a", "principal_type": PRINCIPAL_TYPE_GROUP, "role": ROLE_READER},
            headers=headers,
        )
    ...
    def test_user_by_id_response_consistency(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        # First request
        response1 = test_client.get(ENDPOINT_IDENTITY_USER_DETAIL.format(user_id=user_id), headers=headers)
        assert response1.status_code == status.HTTP_200_OK
        data1 = response1.json()

        # Second request
        response2 = test_client.get(ENDPOINT_IDENTITY_USER_DETAIL.format(user_id=user_id), headers=headers)
        assert response2.status_code == status.HTTP_200_OK
        data2 = response2.json()
    ...
    def test_group_by_id_response_consistency(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        # First request
        response1 = test_client.get(ENDPOINT_IDENTITY_GROUP_DETAIL.format(group_id=group_id), headers=headers)
        assert response1.status_code == status.HTTP_200_OK
        data1 = response1.json()

        # Second request
        response2 = test_client.get(ENDPOINT_IDENTITY_GROUP_DETAIL.format(group_id=group_id), headers=headers)
        assert response2.status_code == status.HTTP_200_OK
        data2 = response2.json()
    ...
    def test_identity_groups_cache_invalidated_on_permission_change(
        ...
        user_token = test_client.create_test_user(
            "cache-user-grp", "Cache User Groups", idp_groups=["test-group-cache"]
        )
        ...
        grant_response = test_client.put(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant_id),
            json={"principal_id": "test-group-cache", "principal_type": PRINCIPAL_TYPE_GROUP, "role": ROLE_READER},
            headers=admin_headers,
        )
        assert grant_response.status_code == status.HTTP_200_OK
    ...
    def test_identity_groups_cache_invalidated_on_permission_removal(
        self, test_client: TestClient, fake_redis_client: Any
    ) -> None:
        ...
        user_token = test_client.create_test_user(
            "cache-user-rmv", "Cache User Remove", idp_groups=["remove-group-cache"]
        )
        user_headers = create_auth_headers(user_token)

        # Admin creates tenant
        create_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Group Remove Cache Test", "description": "Test"}, headers=admin_headers
        )
        tenant_id = create_response.json()["id"]
        ...
        response_with_access = test_client.get(ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=user_headers)
        ...
        response_no_access = test_client.get(ENDPOINT_TENANT_DETAIL.format(tenant_id=tenant_id), headers=user_headers)
    ...
    def test_different_users_different_group_caches(self, test_client: TestClient, fake_redis_client: Any) -> None:
        ...
        # User 1 with group A
        user1_token = test_client.create_test_user("multi-cache-1", "Multi Cache 1", idp_groups=["group-alpha"])
        headers1 = create_auth_headers(user1_token)

        # User 2 with group B
        user2_token = test_client.create_test_user("multi-cache-2", "Multi Cache 2", idp_groups=["group-beta"])
        headers2 = create_auth_headers(user2_token)
        ...
        tenant1_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Tenant Alpha", "description": "For group alpha"}, headers=admin_headers
        )
        tenant1_id = tenant1_response.json()["id"]

        test_client.put(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant1_id),
            json={"principal_id": "group-alpha", "principal_type": PRINCIPAL_TYPE_GROUP, "role": ROLE_READER},
            headers=admin_headers,
        )

        # Tenant 2 - group-beta has access
        tenant2_response = test_client.post(
            ENDPOINT_TENANTS, json={"name": "Tenant Beta", "description": "For group beta"}, headers=admin_headers
        )
        tenant2_id = tenant2_response.json()["id"]

        test_client.put(
            ENDPOINT_TENANT_PRINCIPALS.format(tenant_id=tenant2_id),
            json={"principal_id": "group-beta", "principal_type": PRINCIPAL_TYPE_GROUP, "role": ROLE_READER},
            headers=admin_headers,
        )
    ...
    def test_pagination_parameters_dont_affect_cache_isolation(
        self, test_client: TestClient, fake_redis_client: Any
    ) -> None:
    ...
    def test_search_parameters_dont_affect_cache_isolation(
        self, test_client: TestClient, fake_redis_client: Any
    ) -> None:
