...
class TestN8NAutonomousAgentConfig:
    ...
    def test_missing_workflow_endpoint(self):
        ...
        with pytest.raises(ValidationError):
            N8NAutonomousAgentConfig(api_version="v1", api_api_key_credential_id="cred-123")

    def test_empty_workflow_endpoint(self):
        """Test that empty workflow_endpoint is invalid."""
        with pytest.raises(ValidationError):
            N8NAutonomousAgentConfig(api_version="v1", workflow_endpoint="", api_api_key_credential_id="cred-123")

    def test_missing_credential_id(self):
        """Test that missing api_api_key_credential_id raises ValidationError."""
        with pytest.raises(ValidationError):
            N8NAutonomousAgentConfig(api_version="v1", workflow_endpoint="http://localhost:5678/workflow/test")
...
class TestAutonomousAgentConfigValidatorFactory:
    ...
    def test_get_validator_for_n8n(self):
        ...
        validator = AutonomousAgentConfigValidatorFactory.get_validator(AutonomousAgentTypeEnum.N8N)
    ...
    def test_validate_config_valid(self):
        ...
        result = AutonomousAgentConfigValidatorFactory.validate_config(AutonomousAgentTypeEnum.N8N, config)
    ...
    def test_validate_config_empty_raises_error(self):
        ...
        with pytest.raises(AutonomousAgentConfigValidationError) as exc_info:
            AutonomousAgentConfigValidatorFactory.validate_config(AutonomousAgentTypeEnum.N8N, {})
        assert "required" in str(exc_info.value).lower()

    def test_validate_config_none_raises_error(self):
        """Test that None config raises error."""
        with pytest.raises(AutonomousAgentConfigValidationError):
            AutonomousAgentConfigValidatorFactory.validate_config(AutonomousAgentTypeEnum.N8N, None)
