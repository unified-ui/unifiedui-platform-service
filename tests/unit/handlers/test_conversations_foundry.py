...
class TestConversationHandlerFoundryIntegration:
    ...
    def test_create_conversation_non_foundry_app_no_external_id(self, handler, mock_db_client, mock_user):
        """Test that non-Foundry apps don't get external conversation ID."""
        db_client, session = mock_db_client
        
        # Mock application query
        mock_app = Mock()
        mock_app.type = ApplicationTypeEnum.N8N.value
        mock_app.config = {}
        ...
        mock_conversation.application_id = "app-123"
        ...
        # The second call to scalar_one_or_none returns the application
        ...
        def refresh_mock(obj):
            ...
            for attr in ['id', 'tenant_id', 'application_id', 'ext_conversation_id', 
    ...
    def test_create_conversation_foundry_app_missing_api_key_raises_error(self, handler, mock_db_client, mock_user):
        """Test that Foundry apps without API key raise error."""
        db_client, session = mock_db_client
        
        # Mock application query
        mock_app = Mock()
        mock_app.type = ApplicationTypeEnum.MICROSOFT_FOUNDRY.value
        mock_app.config = {
    ...
    def test_create_conversation_foundry_app_missing_endpoint_raises_error(self, handler, mock_db_client, mock_user):
        """Test that Foundry apps without project endpoint raise error."""
        db_client, session = mock_db_client
        
        # Mock application query - missing project_endpoint
        mock_app = Mock()
        mock_app.type = ApplicationTypeEnum.MICROSOFT_FOUNDRY.value
    ...
    def test_create_conversation_foundry_app_success(self, mock_foundry_client_class, handler, mock_db_client, mock_user):
        ...
        def capture_add(obj):
            ...
            from unifiedui.core.database.models import Conversation
