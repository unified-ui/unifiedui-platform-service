...
class TestResourcePermissionsHandlerConfig:
    ...
    def test_invalid_resource_type_raises_error(self, test_db_client, test_cache_client):
        ...
        handler = ResourcePermissionsHandler(db_client=test_db_client, cache_client=test_cache_client)
...
class TestResourcePermissionsHandlerOperations:
    ...
    def handler(self, test_db_client, test_cache_client):
        ...
        return ResourcePermissionsHandler(db_client=test_db_client, cache_client=test_cache_client)
    ...
    def test_list_permissions_empty(self, handler, setup_tenant_and_chat_agent):
    ...
    def test_list_permissions_resource_not_found(self, handler, setup_tenant_and_chat_agent):
    ...
    def test_set_permission_creates_new_member(self, handler, setup_tenant_and_chat_agent):
    ...
    def test_set_permission_updates_existing_member(self, handler, setup_tenant_and_chat_agent):
    ...
    def test_get_permission(self, handler, setup_tenant_and_chat_agent):
    ...
    def test_get_permission_not_found(self, handler, setup_tenant_and_chat_agent):
    ...
    def test_delete_permission(self, handler, setup_tenant_and_chat_agent):
    ...
    def test_delete_permission_not_found(self, handler, setup_tenant_and_chat_agent):
...
class TestResourcePermissionsHandlerPermissionChecks:
    ...
    def setup_with_member(self, test_db_client, test_db_session):
        """Create tenant, chat agent, and member for testing."""
        tenant_id = str(uuid.uuid4())
        user_id = f"test-user-{str(uuid.uuid4())[:8]}"
        chat_agent_id = str(uuid.uuid4())
    ...
    def test_check_user_permission_resource_admin_bypass(self, handler, setup_with_member):
        ...
        mock_user = Mock()
        mock_user.identity.get_id.return_value = "different-user-id"
        mock_user.tenants = [{"tenant": {"id": data["tenant_id"]}, "roles": [TenantRolesEnum.CHAT_AGENTS_ADMIN.value]}]
        mock_user.groups = []

        # CHAT_AGENTS_ADMIN should have access
        has_access = handler.check_user_permission(
            resource_type="chat_agent",
            tenant_id=data["tenant_id"],
            resource_id=data["chat_agent_id"],
...
class TestResourcePermissionsHandlerFilters:
    ...
    def setup_with_multiple_principals(self, test_db_client, test_db_session):
        """Create tenant, chat agent, and multiple principals for testing filters."""
        tenant_id = str(uuid.uuid4())
        ...
        users = [
            {
                "id": f"user-alpha-{str(uuid.uuid4())[:8]}",
            ...
            {
                "id": f"user-beta-{str(uuid.uuid4())[:8]}",
            ...
            {
                "id": f"user-charlie-{str(uuid.uuid4())[:8]}",
            ...
            {
                "id": f"user-delta-{str(uuid.uuid4())[:8]}",
            ...
            {
                "id": f"user-echo-{str(uuid.uuid4())[:8]}",
                "display_name": "Echo Writer",
                "mail": "echo@test.com",
                "is_active": True,
                "role": PermissionActionEnum.WRITE.value,
            },
        ...
        return {"tenant_id": tenant_id, "chat_agent_id": chat_agent_id, "user_ids": user_ids, "users": users}
