...
class TestHashiCorpVaultClient:
    ...
    def test_initialization(self, mock_vault_class):
        ...
        client = HashiCorpVaultClient(url="http://localhost:8200", token="test-token", mount_point="kv")

        mock_vault_class.assert_called_once_with(url="http://localhost:8200", token="test-token", mount_point="kv")
    ...
    def test_initialization_with_defaults(self, mock_vault_class):
        ...
        HashiCorpVaultClient(url="http://localhost:8200", token="token")
    ...
    def test_with_cache_client(self, mock_vault_class):
        ...
        client = HashiCorpVaultClient(url="http://localhost:8200", token="token", cache_client=mock_cache)
