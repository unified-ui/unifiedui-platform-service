...
from unifiedui.caching.redis.cache import DateTimeEncoder, RedisCache
...
class TestDateTimeEncoder:
    ...
    def test_encodes_nested_datetime(self):
        ...
        data = {"record": {"created_at": dt, "name": "test"}}
        result = json.dumps(data, cls=DateTimeEncoder)
...
class TestRedisCache:
    ...
    def test_initialization_with_custom_params(self, mock_redis_class):
        ...
        cache = RedisCache(host="redis.example.com", port=6380, db=2, password="secret", default_ttl=7200)

        mock_redis_class.assert_called_once_with(
            host="redis.example.com", port=6380, db=2, password="secret", decode_responses=True
        )
