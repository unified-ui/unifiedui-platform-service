"""Okta identity token serializer for JWT claims extraction."""
...
class OktaIdentityTokenSerializer(BaseIdentityToken):
    """Extracts user identity from Okta-issued JWT tokens.

    Okta tokens follow standard OIDC claims: sub, email, name, given_name,
    family_name, preferred_username, plus Okta-specific claims like uid.
    """
    ...
    def __init__(self, token: str, deserialized_token: dict):
        ...
        """
        self._identity_provider = IdenityProviderEnum.OKTA.value
    ...
    def get_id(self) -> str:
        """Return the Okta user ID (uid or sub claim)."""
        return self.deserialized_token.get("uid", self.deserialized_token.get("sub", ""))

    def get_identity_tenant_id(self) -> str:
        """Return the Okta org as tenant ID (extracted from issuer)."""
        iss = self.deserialized_token.get("iss", "")
        if iss.startswith("https://") and ".okta." in iss:
    ...
    def get_principal_name(self) -> str:
        ...
        return self.deserialized_token.get("preferred_username", self.deserialized_token.get("email", ""))
