...
class CacheClientFactory:
    ...
    def create(
        ...
        if cache_type == CacheTypeEnum.REDIS:
            underlying_client = CacheClientFactory._create_redis_client(host, port, db, password, default_ttl)
        else:
    ...
    def _create_redis_client(
        ...
        client = RedisCacheClient(host=host, port=port, db=db, password=password, default_ttl=default_ttl)

        return client


# Convenience function for creating a cache client
def get_cache_client(cache_type: CacheTypeEnum | None = None) -> CacheClient:
    """
