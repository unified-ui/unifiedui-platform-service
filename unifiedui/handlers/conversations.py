...
from unifiedui.schema.responses.conversations import ConversationQuickListItemResponse, ConversationResponse
...
class ConversationHandler:
        ...
        if use_cache and self.cache_client and not has_filters:
            try:
                ...
                if cached_data is not None:
                    logger.debug("Returning cached conversation list")
                    if view == "quick-list":
        ...
        with self.db_client.get_session() as session:
            ...
            if not is_admin:
                ...
                member_subquery = (
                    ...
                    .where(
                        ConversationMember.tenant_id == tenant_id, ConversationMember.principal_id.in_(principal_ids)
                    )
                    .distinct()
                )
            ...
            if view == "quick-list":
                quick_result = [
                    ConversationQuickListItemResponse(id=conv.id, name=conv.name, chat_agent_id=conv.chat_agent_id)
            ...
            if is_admin:
                ...
                if resource_ids:
                    permissions = resolve_my_permissions_bulk(
                        session, ConversationMember, "conversation_id", tenant_id, resource_ids, principal_ids
                    )
                    for r in result:
    ...
    def get_conversation(
        self, tenant_id: str, conversation_id: str, user: ContextIdentityUser | None = None, use_cache: bool = True
    ...
    def create_conversation(
        ...
        For MICROSOFT_FOUNDRY chat agents, this will also create a conversation
        ...
        with self.db_client.get_session() as session:
            # First, check the chat_agent type
            app_query = select(ChatAgent).where(ChatAgent.id == request.chat_agent_id, ChatAgent.tenant_id == tenant_id)
            chat_agent = session.execute(app_query).scalar_one_or_none()
            ...
            if chat_agent.type == ChatAgentTypeEnum.MICROSOFT_FOUNDRY.value and not is_foundry_proxy:
                ...
                # Extract Foundry settings from chat_agent config
                app_config = chat_agent.config or {}
                project_endpoint = app_config.get("project_endpoint")
                api_version = app_config.get("api_version", "2025-11-15-preview")

                if not project_endpoint:
                    raise FoundryConversationCreationError(
                        message="Chat agent config missing 'project_endpoint' for MICROSOFT_FOUNDRY"
                    )

                try:
                    foundry_client = MicrosoftFoundryClient(
                        project_endpoint=project_endpoint, api_token=foundry_api_key, api_version=api_version
                    )
    ...
    def update_conversation(
        self, tenant_id: str, conversation_id: str, request: UpdateConversationRequest, user_id: str
    ) -> ConversationResponse:
        """
    ...
    def list_conversation_permissions(
        ...
        """
        logger.info(
            "Listing conversation permissions", extra={"tenant_id": tenant_id, "conversation_id": conversation_id}
        )
        ...
        return ResourcePrincipalsResponse(
            resource_id=conversation_id, resource_type="conversation", tenant_id=tenant_id, principals=principals
        )

    def get_conversation_permission(
        self, tenant_id: str, conversation_id: str, principal_id: str
    ) -> PrincipalWithRolesResponse:
        """
    ...
    def set_conversation_permission(
        ...
        logger.info(
            ...
            extra={"tenant_id": tenant_id, "conversation_id": conversation_id, "principal_id": request.principal_id},
        )
        ...
        return self.get_conversation_permission(
            tenant_id=tenant_id, conversation_id=conversation_id, principal_id=request.principal_id
        )

    def delete_conversation_permission(
        self, tenant_id: str, conversation_id: str, principal_id: str, principal_type: str, role: str
    ) -> None:
        """
    ...
    def _resolve_user_permission(
        ...
        return resolve_my_permission(
            session, ConversationMember, "conversation_id", tenant_id, conversation_id, principal_ids
        )
    ...
    def _model_to_response(conversation: Conversation) -> ConversationResponse:
        ...
        return ConversationResponse(
            ...
            chat_agent_id=conversation.chat_agent_id,
