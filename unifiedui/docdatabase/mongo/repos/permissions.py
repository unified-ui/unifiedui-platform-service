...
from unifiedui.core.docdatabase.models.permissions import AssignedTo, PermissionModel
...
class MongoDBPermissionsCollection(PermissionsCollection):
    ...
    def _ensure_indexes(self):
        ...
        self._collection.create_index([("tenant_id", 1), ("resource_type", 1), ("resource_id", 1)])

        # Index for user permission lookups
        self._collection.create_index([("tenant_id", 1), ("assigned_to.type", 1), ("assigned_to.id", 1)])

        # Compound index for permission checks
        self._collection.create_index([("tenant_id", 1), ("resource_type", 1), ("resource_id", 1), ("action", 1)])

    def get(self, id: str) -> PermissionModel | None:
    ...
    def get_list(self, filters: dict | None = None, skip: int = 0, limit: int = 100) -> list[PermissionModel]:
    ...
    def get_resource_permissions(self, resource_type: str, resource_id: str, tenant_id: str) -> list[PermissionModel]:
    ...
    def get_user_accessible_resources(
        self, resource_type: str, tenant_id: str, assigned_to_list: list[AssignedTo], action: str | None = None
    ) -> list[str]:
        """
        ...
        for at in assigned_to_list:
            or_conditions.append({"assigned_to.type": at.type, "assigned_to.id": at.id})

        query = {"tenant_id": tenant_id, "resource_type": resource_type, "$or": or_conditions}
    ...
    def check_permission(
        self, resource_type: str, resource_id: str, tenant_id: str, assigned_to_list: list[AssignedTo], action: str
    ) -> bool:
    ...
    def delete_resource_permissions(
        self, resource_type: str, resource_id: str, tenant_id: str, assigned_to: AssignedTo | None = None
    ) -> int:
        """Delete permissions for a resource, optionally filtered by assigned_to."""
        query = {"tenant_id": tenant_id, "resource_type": resource_type, "resource_id": resource_id}
