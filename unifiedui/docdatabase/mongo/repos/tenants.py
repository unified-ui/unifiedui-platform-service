...
class MongoDBTenantsCollection(TenantsCollection):
    ...
    def get(self, id: str) -> TenantModel | None:
    ...
    def get_list(self, filters: dict | None = None, skip: int = 0, limit: int = 100) -> list[TenantModel]:
    ...
    def update(self, id: str, data: dict) -> TenantModel | None:
        """Update an existing tenant."""
        # Update the updated_at timestamp
        data["updated_at"] = current_iso_datetime()

        result = self._collection.find_one_and_update({"id": id}, {"$set": data}, return_document=True)
