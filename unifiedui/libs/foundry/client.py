...
class MicrosoftFoundryClient:
    ...
    def create_conversation(self, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        logger.info("Creating Foundry conversation", extra={
        ...
        try:
            ...
            logger.info("Foundry conversation created", extra={
            ...
            logger.error(
                f"Failed to create Foundry conversation: HTTP {status_code}",
            ...
            raise MicrosoftFoundryError(
                message=f"Failed to create Foundry conversation: {str(e)}"
            ) from e
    
    def get_conversation_id(self, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        if not conversation_id:
            raise MicrosoftFoundryError(
                message="Foundry response missing conversation ID"
            )
