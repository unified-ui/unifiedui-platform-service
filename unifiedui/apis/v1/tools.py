...
from unifiedui.exc.tools import ToolNotFoundError, InvalidToolCredentialError
...
async def list_tools(
    ...
    view: Optional[ListViewEnum] = Query(None, description="View type: 'full' (default) or 'quick-list'"),
...
async def list_tool_permissions(
    ...
    roles: Optional[str] = Query(None, description="Comma-separated list of roles to filter by (e.g., 'READ,WRITE,ADMIN')"),
    is_active: Optional[bool] = Query(None, description="Filter by is_active status"),
    order_by: Optional[str] = Query(None, description="Column to order by (e.g., 'display_name', 'principal_name', 'mail', 'created_at')"),
    order_direction: Optional[OrderDirectionEnum] = Query(None, description="Sort direction: 'asc' or 'desc'"),
    handler: ToolHandler = Depends(get_tool_handler)
) -> ResourcePrincipalsResponse:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to list tool permissions"
        )
...
async def get_tool_permission(
    ...
    try:
        ...
        return handler.get_tool_permission(
            tenant_id=tenant_id,
            tool_id=tool_id,
            principal_id=principal_id
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get tool permission"
        )
...
async def set_tool_permission(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to set tool permission"
        )
...
async def delete_tool_permission(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to delete tool permission"
        )
